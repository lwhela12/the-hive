-- Migration: 039_group_dm_policies_and_functions.sql
-- Adds policies and functions for group DMs
-- (Separate from 038 because PostgreSQL requires enum values to be committed before use)

-- ============================================
-- RLS POLICY FOR CREATING GROUP DM ROOMS
-- ============================================

-- Allow members to create group_dm rooms (same as dm rooms)
DROP POLICY IF EXISTS "Members can create DM rooms" ON public.chat_rooms;

CREATE POLICY "Members can create DM and group DM rooms" ON public.chat_rooms
  FOR INSERT WITH CHECK (
    room_type::text IN ('dm', 'group_dm')
    AND EXISTS (
...
-- ============================================
-- FUNCTION: GET OR CREATE GROUP DM ROOM
-- ============================================

-- Finds or creates a group DM room with exactly the specified participants
CREATE OR REPLACE FUNCTION get_or_create_group_dm_room(
  p_community_id uuid,
  p_user_ids uuid[]
) RETURNS uuid AS $$
DECLARE
...
-- ============================================
-- UPDATE get_chat_rooms_with_data RPC
-- ============================================

-- Update the optimized RPC to include group_dm rooms
...
BEGIN
  RETURN QUERY
  WITH user_rooms AS (
    -- Get all rooms user has access to (community rooms + DMs/group_dms they're in)
  ...
  room_members_data AS (
    -- Get members for DM and group_dm rooms with user profiles
    SELECT
