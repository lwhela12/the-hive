...
async function fetchBoardPostIndex(
  ...
  const { data: posts, error } = await supabase
    ...
    .select(`
      ...
      reply_count,
      created_at,
      category:board_categories(name, topic_kind, goal_title, status, owner:profiles!board_categories_owner_user_id_fkey(name)),
  ...
  const result = posts.map((p: any) => ({
    ...
    board_status: p.category?.status || 'active',
...
Status: ${qb.status}`);
    ...
    if (data.communityContext.publicWishes.length > 0) {
      const wishes = data.communityContext.publicWishes
        ...
        .join('\n');
      sections.push(`## Active Public Wishes in HIVE
${wishes}`);
    }
