/**
 * The letter parser, for the email side of the wall.
 *
 * THIS IS A DELIBERATE SECOND COPY of `readLetter` from
 * `hive-app/lib/newsletterHeaders.ts`. **Change one, change the other.**
 *
 * It exists because the two runtimes genuinely cannot share a file. The app
 * is TypeScript compiled by Metro; edge functions are Deno, and
 * `tsconfig.json` excludes `supabase/functions` from the app's build on
 * purpose. Importing the app's copy from here got as far as deploying — the
 * CLI happily uploaded `lib/newsletterHeaders.ts` alongside the function —
 * and then the worker failed to boot on every single request, because the
 * module graph does not really reach outside the functions directory at
 * runtime. Tried on 2026-08-12; the boundary is real.
 *
 * The copy is worth it: `send-newsletter` has to lay out an issue exactly
 * the way The Buzz screen and the letter screen do, or Nat's *"whatever is
 * in the email [is] on HIVE wide & public site... nothing gets lost"*
 * quietly stops being true for headings, dates and lists.
 *
 * Nothing below is edited. It is the app's function, verbatim, so a diff
 * between the two files shows drift immediately.
 */
