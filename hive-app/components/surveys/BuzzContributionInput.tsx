...
export function BuzzContributionInput({ value, onChangeText, placeholder, disabled }: {
  ...
  // The Buzz is HIVE-Wide editorial space. Production is a sealed HIVE: its
  // name, members and tag target must never appear in this form.
  const buzzMemberships = memberships.filter(membership => membership.community?.slug !== 'show');
  const ids = buzzMemberships.map(membership => membership.community_id).sort();
  ...
  return <View><ComposerBar tone="light" variant="form" containerClassName="mt-2" multiline minHeight={100}
    ...
    mentionPicker="bubbles" mentionReach={{ reach: 'all_hives', otherHives: taggableHivesFromMemberships(buzzMemberships), offerOtherHives: true }} />
