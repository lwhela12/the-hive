...
(async()=>{
 ...
 for(const community of [null,'og']){
  ...
  const db={from(table){const ops=[];queries.push({table,ops});const q={};for(const key of ['select','eq','or','order','in'])q[key]=(...args)=>{ops.push([key,...args]);return q;};q.then=fn=>Promise.resolve({data:table==='surveys'?surveys:table==='profiles'?[{id:'person',full_name:'Respondent Name'}]:[{id:'r',user_id:'person',answers:{q:'Preserved answer'},response_period:'2026-09'}]}).then(fn);return q;}};
  ...
  const responseQuery=queries.find(q=>q.table==='survey_responses');assert.ok(responseQuery.ops.some(op=>community?op[0]==='or'&&op[1]==='community_id.eq.og,community_id.is.null':op[0]==='eq'&&op[1]==='community_id'&&op[2]==='og'));
