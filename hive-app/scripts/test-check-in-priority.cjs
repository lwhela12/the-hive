...
assert.match(screen, /originMembership\?\.community_id \?\? null/, 'a check-in keeps the HIVE where it was opened');
assert.match(screen, /if \(!isFocused\) \{ originHandled\.current = null; return; \}/, 'returning to the same HIVE reopens its section');
assert.doesNotMatch(screen, /hiveSlug=\{memberships\.find\(m => m\.community_id === selected\)/, 'moving between sections does not change visual HIVE context');
