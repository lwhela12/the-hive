...
assert.match(home, /setEditingEvent\(isGeneratedDuesReminder \? null : event\)/, 'Generated reminder opens as a create-on-save event draft');
assert.match(home, /setEventDraftSource\(isGeneratedDuesReminder \? 'dues-reminder' : null\)/, 'Generated reminder keeps edit wording while materializing');
assert.match(home, /const isEditingDraft = !!editingEvent \|\| eventDraftSource === 'dues-reminder'/, 'Generated reminder uses Edit and Save labels');
assert.doesNotMatch(home, /if \(isDuesReminder\)[\s\S]{0,120}router\.push\('\/honey-pot'/, 'Generated reminder does not redirect to Honey Pot');
